...
LOGS = logging.getLogger("تليثون سوي \n ")
...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/9446ca7b48e18c10690c2.jpg", caption="🝳 ⦙ تـمّ  اعـادة تشـغيل\n تليثـون سوي✓  :  [ 7.7 ] .\n\n🝳 ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n🝳 ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n🝳 ⦙ القناة الرسمية تليثون سوي : @IQTHON\n🝳 ⦙ فارات سورس تليثون  :@TEAMTELETHON \n🝳 ⦙ كلايش تليثون :  @FGFFG\n 🝳 ⦙التحديثات والاضافات :  @U_7h1 \n",                buttons=[(Button.url("مطور تليثون الرسمي", "https://t.me/U_7h1"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون سوي العام", iqthon, Config.TG_BOT_USERNAME, descript  ,  iqphoto1 )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(            "تخزين تليثون سوي الخاص", iqthon, Config.TG_BOT_USERNAME, descript    , iqphoto2  )
